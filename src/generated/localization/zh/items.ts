/**
 * Item translations for ZH locale
 * Generated on 2025-07-26T21:25:23.352Z
 */

import type { ItemHrid } from '../../game-logic/items.js'

export const itemTranslations: Record<ItemHrid, { name: string; description?: string }> = {
  '/items/abyssal_essence': {
    name: '地狱精华',
    description: '用于强化地狱深渊特殊装备的材料',
  },
  '/items/acrobatic_hood': {
    name: '杂技师兜帽',
  },
  '/items/acrobats_ribbon': {
    name: '杂技师彩带',
    description: '用于缝纫杂技师兜帽的材料',
  },
  '/items/advanced_task_badge': {
    name: '高级任务徽章',
  },
  '/items/alchemists_bottoms': {
    name: '炼金师下装',
  },
  '/items/alchemists_top': {
    name: '炼金师上衣',
  },
  '/items/alchemy_essence': {
    name: '炼金精华',
    description: '用于冲泡炼金茶和制作炼金催化剂',
  },
  '/items/alchemy_tea': {
    name: '炼金茶',
  },
  '/items/amber': {
    name: '琥珀',
    description: '一种闪亮的宝石，通常在宝箱中找到',
  },
  '/items/amethyst': {
    name: '紫水晶',
    description: '一种闪亮的宝石，通常在宝箱中找到',
  },
  '/items/anchorbound_plate_body': {
    name: '锚定胸甲',
  },
  '/items/anchorbound_plate_legs': {
    name: '锚定腿甲',
  },
  '/items/apple': {
    name: '苹果',
  },
  '/items/apple_gummy': {
    name: '苹果软糖',
  },
  '/items/apple_yogurt': {
    name: '苹果酸奶',
  },
  '/items/aqua_arrow': {
    name: '流水箭',
  },
  '/items/aqua_aura': {
    name: '流水光环',
  },
  '/items/aqua_essence': {
    name: '海洋精华',
    description: '用于强化海洋星球特殊装备的材料',
  },
  '/items/arabica_coffee_bean': {
    name: '低级咖啡豆',
  },
  '/items/arcane_bow': {
    name: '神秘弓',
  },
  '/items/arcane_crossbow': {
    name: '神秘弩',
  },
  '/items/arcane_fire_staff': {
    name: '神秘火法杖',
  },
  '/items/arcane_log': {
    name: '神秘原木',
  },
  '/items/arcane_lumber': {
    name: '神秘木板',
  },
  '/items/arcane_nature_staff': {
    name: '神秘自然法杖',
  },
  '/items/arcane_reflection': {
    name: '奥术反射',
  },
  '/items/arcane_shield': {
    name: '神秘盾',
  },
  '/items/arcane_water_staff': {
    name: '神秘水法杖',
  },
  '/items/artisan_tea': {
    name: '工匠茶',
  },
  '/items/attack_coffee': {
    name: '攻击咖啡',
  },
  '/items/azure_alembic': {
    name: '蔚蓝蒸馏器',
  },
  '/items/azure_boots': {
    name: '蔚蓝靴',
  },
  '/items/azure_brush': {
    name: '蔚蓝刷子',
  },
  '/items/azure_buckler': {
    name: '蔚蓝圆盾',
  },
  '/items/azure_bulwark': {
    name: '蔚蓝重盾',
  },
  '/items/azure_cheese': {
    name: '蔚蓝奶酪',
  },
  '/items/azure_chisel': {
    name: '蔚蓝凿子',
  },
  '/items/azure_enhancer': {
    name: '蔚蓝强化器',
  },
  '/items/azure_gauntlets': {
    name: '蔚蓝护手',
  },
  '/items/azure_hammer': {
    name: '蔚蓝锤子',
  },
  '/items/azure_hatchet': {
    name: '蔚蓝斧头',
  },
  '/items/azure_helmet': {
    name: '蔚蓝头盔',
  },
  '/items/azure_mace': {
    name: '蔚蓝钉头锤',
  },
  '/items/azure_milk': {
    name: '蔚蓝牛奶',
    description: '哞哞哞',
  },
  '/items/azure_needle': {
    name: '蔚蓝针',
  },
  '/items/azure_plate_body': {
    name: '蔚蓝胸甲',
  },
  '/items/azure_plate_legs': {
    name: '蔚蓝腿甲',
  },
  '/items/azure_pot': {
    name: '蔚蓝壶',
  },
  '/items/azure_shears': {
    name: '蔚蓝剪刀',
  },
  '/items/azure_spatula': {
    name: '蔚蓝锅铲',
  },
  '/items/azure_spear': {
    name: '蔚蓝长枪',
  },
  '/items/azure_sword': {
    name: '蔚蓝剑',
  },
  '/items/bag_of_10_cowbells': {
    name: '牛铃袋 (10个)',
    description: '可交易的一袋牛铃，每袋包含10个牛铃。只能整袋交易，一旦打开将无法出售',
  },
  '/items/bamboo_boots': {
    name: '竹靴',
  },
  '/items/bamboo_branch': {
    name: '竹子',
  },
  '/items/bamboo_fabric': {
    name: '竹子布料',
  },
  '/items/bamboo_gloves': {
    name: '竹手套',
  },
  '/items/bamboo_hat': {
    name: '竹帽',
  },
  '/items/bamboo_robe_bottoms': {
    name: '竹袍裙',
  },
  '/items/bamboo_robe_top': {
    name: '竹袍服',
  },
  '/items/basic_task_badge': {
    name: '基础任务徽章',
  },
  '/items/bear_essence': {
    name: '熊熊精华',
    description: '用于强化熊熊星球特殊装备的材料',
  },
  '/items/beast_boots': {
    name: '野兽靴',
  },
  '/items/beast_bracers': {
    name: '野兽护腕',
  },
  '/items/beast_chaps': {
    name: '野兽皮裤',
  },
  '/items/beast_hide': {
    name: '野兽皮',
  },
  '/items/beast_hood': {
    name: '野兽兜帽',
  },
  '/items/beast_leather': {
    name: '野兽皮革',
  },
  '/items/beast_tunic': {
    name: '野兽皮衣',
  },
  '/items/berserk': {
    name: '狂暴',
  },
  '/items/birch_bow': {
    name: '桦木弓',
  },
  '/items/birch_crossbow': {
    name: '桦木弩',
  },
  '/items/birch_fire_staff': {
    name: '桦木火法杖',
  },
  '/items/birch_log': {
    name: '白桦原木',
  },
  '/items/birch_lumber': {
    name: '白桦木板',
  },
  '/items/birch_nature_staff': {
    name: '桦木自然法杖',
  },
  '/items/birch_shield': {
    name: '桦木盾',
  },
  '/items/birch_water_staff': {
    name: '桦木水法杖',
  },
  '/items/bishops_codex': {
    name: '主教法典',
  },
  '/items/bishops_scroll': {
    name: '主教卷轴',
    description: '用于制作主教之书的材料',
  },
  '/items/black_bear_fluff': {
    name: '黑熊绒',
    description: '用于锻造黑熊鞋的材料',
  },
  '/items/black_bear_shoes': {
    name: '黑熊鞋',
  },
  '/items/black_tea_leaf': {
    name: '黑茶叶',
  },
  '/items/blackberry': {
    name: '黑莓',
  },
  '/items/blackberry_cake': {
    name: '黑莓蛋糕',
  },
  '/items/blackberry_donut': {
    name: '黑莓甜甜圈',
  },
  '/items/blazing_trident': {
    name: '炽焰三叉戟',
  },
  '/items/blessed_tea': {
    name: '福气茶',
  },
  '/items/blooming_trident': {
    name: '绽放三叉戟',
  },
  '/items/blue_key_fragment': {
    name: '蓝色钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/blueberry': {
    name: '蓝莓',
  },
  '/items/blueberry_cake': {
    name: '蓝莓蛋糕',
  },
  '/items/blueberry_donut': {
    name: '蓝莓甜甜圈',
  },
  '/items/branch_of_insight': {
    name: '洞察之枝',
    description: '用于制作特殊专业工具和服装的材料',
  },
  '/items/brewers_bottoms': {
    name: '饮品师下装',
  },
  '/items/brewers_top': {
    name: '饮品师上衣',
  },
  '/items/brewing_essence': {
    name: '冲泡精华',
    description: '用于冲泡冲泡茶和制作炼金催化剂',
  },
  '/items/brewing_tea': {
    name: '冲泡茶',
  },
  '/items/brown_key_fragment': {
    name: '棕色钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/burble_alembic': {
    name: '深紫蒸馏器',
  },
  '/items/burble_boots': {
    name: '深紫靴',
  },
  '/items/burble_brush': {
    name: '深紫刷子',
  },
  '/items/burble_buckler': {
    name: '深紫圆盾',
  },
  '/items/burble_bulwark': {
    name: '深紫重盾',
  },
  '/items/burble_cheese': {
    name: '深紫奶酪',
  },
  '/items/burble_chisel': {
    name: '深紫凿子',
  },
  '/items/burble_enhancer': {
    name: '深紫强化器',
  },
  '/items/burble_gauntlets': {
    name: '深紫护手',
  },
  '/items/burble_hammer': {
    name: '深紫锤子',
  },
  '/items/burble_hatchet': {
    name: '深紫斧头',
  },
  '/items/burble_helmet': {
    name: '深紫头盔',
  },
  '/items/burble_mace': {
    name: '深紫钉头锤',
  },
  '/items/burble_milk': {
    name: '深紫牛奶',
    description: '哞哞哞哞',
  },
  '/items/burble_needle': {
    name: '深紫针',
  },
  '/items/burble_plate_body': {
    name: '深紫胸甲',
  },
  '/items/burble_plate_legs': {
    name: '深紫腿甲',
  },
  '/items/burble_pot': {
    name: '深紫壶',
  },
  '/items/burble_shears': {
    name: '深紫剪刀',
  },
  '/items/burble_spatula': {
    name: '深紫锅铲',
  },
  '/items/burble_spear': {
    name: '深紫长枪',
  },
  '/items/burble_sword': {
    name: '深紫剑',
  },
  '/items/burble_tea_leaf': {
    name: '紫茶叶',
  },
  '/items/burning_key_fragment': {
    name: '燃烧钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/butter_of_proficiency': {
    name: '精通之油',
    description: '用于制作特殊专业工具和服装的材料',
  },
  '/items/catalyst_of_coinification': {
    name: '点金催化剂',
    description: '在炼金时使用，可提高15%的点金成功率 (乘法)。成功时消耗一个催化剂。',
  },
  '/items/catalyst_of_decomposition': {
    name: '分解催化剂',
    description: '在炼金时使用，可将分解成功率提高 15% (乘法)。成功时消耗一个催化剂。',
  },
  '/items/catalyst_of_transmutation': {
    name: '转化催化剂',
    description: '在炼金时使用，可将转化成功率提高 15% (乘法)。成功时消耗一个催化剂。',
  },
  '/items/catalytic_tea': {
    name: '催化茶',
  },
  '/items/cedar_bow': {
    name: '雪松弓',
  },
  '/items/cedar_crossbow': {
    name: '雪松弩',
  },
  '/items/cedar_fire_staff': {
    name: '雪松火法杖',
  },
  '/items/cedar_log': {
    name: '雪松原木',
  },
  '/items/cedar_lumber': {
    name: '雪松木板',
  },
  '/items/cedar_nature_staff': {
    name: '雪松自然法杖',
  },
  '/items/cedar_shield': {
    name: '雪松盾',
  },
  '/items/cedar_water_staff': {
    name: '雪松水法杖',
  },
  '/items/celestial_alembic': {
    name: '星空蒸馏器',
  },
  '/items/celestial_brush': {
    name: '星空刷子',
  },
  '/items/celestial_chisel': {
    name: '星空凿子',
  },
  '/items/celestial_enhancer': {
    name: '星空强化器',
  },
  '/items/celestial_hammer': {
    name: '星空锤子',
  },
  '/items/celestial_hatchet': {
    name: '星空斧头',
  },
  '/items/celestial_needle': {
    name: '星空针',
  },
  '/items/celestial_pot': {
    name: '星空壶',
  },
  '/items/celestial_shears': {
    name: '星空剪刀',
  },
  '/items/celestial_spatula': {
    name: '星空锅铲',
  },
  '/items/centaur_boots': {
    name: '半人马靴',
  },
  '/items/centaur_hoof': {
    name: '半人马蹄',
    description: '用于缝纫半人马靴的材料',
  },
  '/items/channeling_coffee': {
    name: '吟唱咖啡',
  },
  '/items/chaotic_chain': {
    name: '混沌锁链',
    description: '用于锻造混沌连枷的材料',
  },
  '/items/chaotic_flail': {
    name: '混沌连枷',
  },
  '/items/cheese': {
    name: '奶酪',
  },
  '/items/cheese_alembic': {
    name: '奶酪蒸馏器',
  },
  '/items/cheese_boots': {
    name: '奶酪靴',
  },
  '/items/cheese_brush': {
    name: '奶酪刷子',
  },
  '/items/cheese_buckler': {
    name: '奶酪圆盾',
  },
  '/items/cheese_bulwark': {
    name: '奶酪重盾',
  },
  '/items/cheese_chisel': {
    name: '奶酪凿子',
  },
  '/items/cheese_enhancer': {
    name: '奶酪强化器',
  },
  '/items/cheese_gauntlets': {
    name: '奶酪护手',
  },
  '/items/cheese_hammer': {
    name: '奶酪锤子',
  },
  '/items/cheese_hatchet': {
    name: '奶酪斧头',
  },
  '/items/cheese_helmet': {
    name: '奶酪头盔',
  },
  '/items/cheese_mace': {
    name: '奶酪钉头锤',
  },
  '/items/cheese_needle': {
    name: '奶酪针',
  },
  '/items/cheese_plate_body': {
    name: '奶酪胸甲',
  },
  '/items/cheese_plate_legs': {
    name: '奶酪腿甲',
  },
  '/items/cheese_pot': {
    name: '奶酪壶',
  },
  '/items/cheese_shears': {
    name: '奶酪剪刀',
  },
  '/items/cheese_spatula': {
    name: '奶酪锅铲',
  },
  '/items/cheese_spear': {
    name: '奶酪长枪',
  },
  '/items/cheese_sword': {
    name: '奶酪剑',
  },
  '/items/cheesemakers_bottoms': {
    name: '奶酪师下装',
  },
  '/items/cheesemakers_top': {
    name: '奶酪师上衣',
  },
  '/items/cheesesmithing_essence': {
    name: '奶酪锻造精华',
    description: '用于冲泡奶酪锻造茶和制作炼金催化剂',
  },
  '/items/cheesesmithing_tea': {
    name: '奶酪锻造茶',
  },
  '/items/chefs_bottoms': {
    name: '厨师下装',
  },
  '/items/chefs_top': {
    name: '厨师上衣',
  },
  '/items/chimerical_chest': {
    name: '奇幻宝箱',
    description: '攻克【奇幻洞穴】后的奖励，可以用【奇幻宝箱钥匙】打开',
  },
  '/items/chimerical_chest_key': {
    name: '奇幻宝箱钥匙',
    description: '开启一个奇幻宝箱',
  },
  '/items/chimerical_entry_key': {
    name: '奇幻钥匙',
    description: '允许进入地下城【奇幻洞穴】1次',
  },
  '/items/chimerical_essence': {
    name: '奇幻精华',
    description: '用于强化奇幻洞穴特殊装备的材料',
  },
  '/items/chimerical_quiver': {
    name: '奇幻箭袋',
  },
  '/items/chimerical_token': {
    name: '奇幻代币',
    description: '来自【奇幻洞穴】的地下城代币。可以在地下城商店里消费',
  },
  '/items/chrono_gloves': {
    name: '时空手套',
  },
  '/items/chrono_sphere': {
    name: '时空球',
    description: '用于缝纫附魔手套或时空手套的材料',
  },
  '/items/cleave': {
    name: '分裂斩',
  },
  '/items/cocoon': {
    name: '蚕茧',
  },
  '/items/coin': {
    name: '金币',
    description: '基础货币',
  },
  '/items/collectors_boots': {
    name: '收藏家靴',
  },
  '/items/colossus_core': {
    name: '巨像核心',
    description: '用于锻造巨像胸甲或腿甲的材料',
  },
  '/items/colossus_plate_body': {
    name: '巨像胸甲',
  },
  '/items/colossus_plate_legs': {
    name: '巨像腿甲',
  },
  '/items/cooking_essence': {
    name: '烹饪精华',
    description: '用于冲泡烹饪茶和制作炼金催化剂',
  },
  '/items/cooking_tea': {
    name: '烹饪茶',
  },
  '/items/corsair_crest': {
    name: '掠夺者徽章',
    description: '用于锻造掠夺者头盔的材料',
  },
  '/items/corsair_helmet': {
    name: '掠夺者头盔',
  },
  '/items/cotton': {
    name: '棉花',
  },
  '/items/cotton_boots': {
    name: '棉靴',
  },
  '/items/cotton_fabric': {
    name: '棉花布料',
  },
  '/items/cotton_gloves': {
    name: '棉手套',
  },
  '/items/cotton_hat': {
    name: '棉帽',
  },
  '/items/cotton_robe_bottoms': {
    name: '棉袍裙',
  },
  '/items/cotton_robe_top': {
    name: '棉布袍服',
  },
  '/items/cowbell': {
    name: '牛铃',
    description: '高级货币。可在牛铃商店购买或使用这些货币',
  },
  '/items/crab_pincer': {
    name: '蟹钳',
    description: '用于锻造蟹钳手套的材料',
  },
  '/items/crafters_bottoms': {
    name: '工匠下装',
  },
  '/items/crafters_top': {
    name: '工匠上衣',
  },
  '/items/crafting_essence': {
    name: '制作精华',
    description: '用于冲泡制作茶和制作炼金催化剂',
  },
  '/items/crafting_tea': {
    name: '制作茶',
  },
  '/items/crimson_alembic': {
    name: '绛红蒸馏器',
  },
  '/items/crimson_boots': {
    name: '绛红靴',
  },
  '/items/crimson_brush': {
    name: '绛红刷子',
  },
  '/items/crimson_buckler': {
    name: '绛红圆盾',
  },
  '/items/crimson_bulwark': {
    name: '绛红重盾',
  },
  '/items/crimson_cheese': {
    name: '绛红奶酪',
  },
  '/items/crimson_chisel': {
    name: '绛红凿子',
  },
  '/items/crimson_enhancer': {
    name: '绛红强化器',
  },
  '/items/crimson_gauntlets': {
    name: '绛红护手',
  },
  '/items/crimson_hammer': {
    name: '绛红锤子',
  },
  '/items/crimson_hatchet': {
    name: '绛红斧头',
  },
  '/items/crimson_helmet': {
    name: '绛红头盔',
  },
  '/items/crimson_mace': {
    name: '绛红钉头锤',
  },
  '/items/crimson_milk': {
    name: '绛红牛奶',
    description: '哞哞哞哞哞',
  },
  '/items/crimson_needle': {
    name: '绛红针',
  },
  '/items/crimson_plate_body': {
    name: '绛红胸甲',
  },
  '/items/crimson_plate_legs': {
    name: '绛红腿甲',
  },
  '/items/crimson_pot': {
    name: '绛红壶',
  },
  '/items/crimson_shears': {
    name: '绛红剪刀',
  },
  '/items/crimson_spatula': {
    name: '绛红锅铲',
  },
  '/items/crimson_spear': {
    name: '绛红长枪',
  },
  '/items/crimson_sword': {
    name: '绛红剑',
  },
  '/items/crippling_slash': {
    name: '致残斩',
  },
  '/items/critical_aura': {
    name: '暴击光环',
  },
  '/items/critical_coffee': {
    name: '暴击咖啡',
  },
  '/items/crushed_amber': {
    name: '琥珀碎片',
    description: '曾经是一块琥珀',
  },
  '/items/crushed_amethyst': {
    name: '紫水晶碎片',
    description: '曾经是一颗紫水晶',
  },
  '/items/crushed_garnet': {
    name: '石榴石碎片',
    description: '曾经是一颗石榴石',
  },
  '/items/crushed_jade': {
    name: '翡翠碎片',
    description: '曾经是一块翡翠',
  },
  '/items/crushed_moonstone': {
    name: '月亮石碎片',
    description: '曾经是一枚月亮石',
  },
  '/items/crushed_pearl': {
    name: '珍珠碎片',
    description: '曾经是一粒珍珠',
  },
  '/items/crushed_philosophers_stone': {
    name: '贤者之石碎片',
    description: '曾经是一块贤者之石',
  },
  '/items/crushed_sunstone': {
    name: '太阳石碎片',
    description: '曾经是一枚太阳石',
  },
  '/items/cupcake': {
    name: '纸杯蛋糕',
  },
  '/items/cursed_ball': {
    name: '诅咒之球',
    description: '用于制作咒怨之弓的材料',
  },
  '/items/cursed_bow': {
    name: '咒怨之弓',
  },
  '/items/dairyhands_bottoms': {
    name: '挤奶工下装',
  },
  '/items/dairyhands_top': {
    name: '挤奶工上衣',
  },
  '/items/damaged_anchor': {
    name: '破损船锚',
    description: '用于锻造锚定胸甲或腿甲的材料',
  },
  '/items/dark_key_fragment': {
    name: '黑暗钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/defense_coffee': {
    name: '防御咖啡',
  },
  '/items/demonic_core': {
    name: '恶魔核心',
    description: '用于锻造恶魔胸甲或腿甲的材料',
  },
  '/items/demonic_plate_body': {
    name: '恶魔胸甲',
  },
  '/items/demonic_plate_legs': {
    name: '恶魔腿甲',
  },
  '/items/dodocamel_gauntlets': {
    name: '渡渡驼护手',
  },
  '/items/dodocamel_plume': {
    name: '渡渡驼之翎',
    description: '用于锻造渡渡驼护手的材料',
  },
  '/items/donut': {
    name: '甜甜圈',
  },
  '/items/dragon_fruit': {
    name: '火龙果',
  },
  '/items/dragon_fruit_gummy': {
    name: '火龙果软糖',
  },
  '/items/dragon_fruit_yogurt': {
    name: '火龙果酸奶',
  },
  '/items/earrings_of_armor': {
    name: '护甲耳环',
  },
  '/items/earrings_of_critical_strike': {
    name: '暴击耳环',
  },
  '/items/earrings_of_essence_find': {
    name: '精华发现耳环',
  },
  '/items/earrings_of_gathering': {
    name: '采集耳环',
  },
  '/items/earrings_of_rare_find': {
    name: '稀有发现耳环',
  },
  '/items/earrings_of_regeneration': {
    name: '恢复耳环',
  },
  '/items/earrings_of_resistance': {
    name: '抗性耳环',
  },
  '/items/efficiency_tea': {
    name: '效率茶',
  },
  '/items/egg': {
    name: '鸡蛋',
  },
  '/items/elemental_affinity': {
    name: '元素增幅',
  },
  '/items/elusiveness': {
    name: '闪避',
  },
  '/items/emp_tea_leaf': {
    name: '虚空茶叶',
  },
  '/items/enchanted_chest': {
    name: '秘法宝箱',
    description: '攻克【秘法要塞】后的奖励，可以用【秘法宝箱钥匙】打开',
  },
  '/items/enchanted_chest_key': {
    name: '秘法宝箱钥匙',
    description: '开启一个秘法宝箱',
  },
  '/items/enchanted_cloak': {
    name: '秘法披风',
  },
  '/items/enchanted_entry_key': {
    name: '秘法钥匙',
    description: '允许进入地下城【秘法要塞】1次',
  },
  '/items/enchanted_essence': {
    name: '秘法精华',
    description: '用于强化秘法要塞特殊装备的材料',
  },
  '/items/enchanted_gloves': {
    name: '附魔手套',
  },
  '/items/enchanted_token': {
    name: '秘法代币',
    description: '来自【秘法要塞】的地下城代币。可以在地下城商店里消费',
  },
  '/items/enhancers_bottoms': {
    name: '强化师下装',
  },
  '/items/enhancers_top': {
    name: '强化师上衣',
  },
  '/items/enhancing_essence': {
    name: '强化精华',
    description: '用于冲泡强化茶和制作炼金催化剂',
  },
  '/items/enhancing_tea': {
    name: '强化茶',
  },
  '/items/entangle': {
    name: '缠绕',
  },
  '/items/excelsa_coffee_bean': {
    name: '特级咖啡豆',
  },
  '/items/expert_task_badge': {
    name: '专家任务徽章',
  },
  '/items/eye_of_the_watcher': {
    name: '观察者之眼',
    description: '用于制作掌上监工或警戒遗物的材料',
  },
  '/items/eye_watch': {
    name: '掌上监工',
  },
  '/items/eyessence': {
    name: '眼精华',
    description: '用于强化眼球星球特殊装备的材料',
  },
  '/items/fierce_aura': {
    name: '物理光环',
  },
  '/items/fieriosa_coffee_bean': {
    name: '火山咖啡豆',
  },
  '/items/fighter_necklace': {
    name: '战士项链',
  },
  '/items/fireball': {
    name: '火球',
  },
  '/items/firestorm': {
    name: '火焰风暴',
  },
  '/items/flame_arrow': {
    name: '烈焰箭',
  },
  '/items/flame_aura': {
    name: '火焰光环',
  },
  '/items/flame_blast': {
    name: '熔岩爆裂',
  },
  '/items/flaming_cloth': {
    name: '烈焰织物',
    description: '用于缝纫烈焰袍服或袍裙的材料',
  },
  '/items/flaming_robe_bottoms': {
    name: '烈焰袍裙',
  },
  '/items/flaming_robe_top': {
    name: '烈焰袍服',
  },
  '/items/flax': {
    name: '亚麻',
  },
  '/items/fluffy_red_hat': {
    name: '蓬松红帽子',
  },
  '/items/foragers_bottoms': {
    name: '采摘者下装',
  },
  '/items/foragers_top': {
    name: '采摘者上衣',
  },
  '/items/foraging_essence': {
    name: '采摘精华',
    description: '用于冲泡采摘茶和制作炼金催化剂',
  },
  '/items/foraging_tea': {
    name: '采摘茶',
  },
  '/items/fracturing_impact': {
    name: '碎裂冲击',
  },
  '/items/frenzy': {
    name: '狂速',
  },
  '/items/frost_sphere': {
    name: '冰霜球',
    description: '用于制作冰霜法杖的材料',
  },
  '/items/frost_staff': {
    name: '冰霜法杖',
  },
  '/items/frost_surge': {
    name: '冰霜爆裂',
  },
  '/items/furious_spear': {
    name: '狂怒长枪',
  },
  '/items/garnet': {
    name: '石榴石',
    description: '一种闪亮的宝石，通常在宝箱中找到',
  },
  '/items/gathering_tea': {
    name: '采集茶',
  },
  '/items/gator_vest': {
    name: '鳄鱼马甲',
  },
  '/items/giant_pouch': {
    name: '巨大袋子',
  },
  '/items/ginkgo_bow': {
    name: '银杏弓',
  },
  '/items/ginkgo_crossbow': {
    name: '银杏弩',
  },
  '/items/ginkgo_fire_staff': {
    name: '银杏火法杖',
  },
  '/items/ginkgo_log': {
    name: '银杏原木',
  },
  '/items/ginkgo_lumber': {
    name: '银杏木板',
  },
  '/items/ginkgo_nature_staff': {
    name: '银杏自然法杖',
  },
  '/items/ginkgo_shield': {
    name: '银杏盾',
  },
  '/items/ginkgo_water_staff': {
    name: '银杏水法杖',
  },
  '/items/gluttonous_energy': {
    name: '贪食能量',
    description: '用于缝纫贪食之袋',
  },
  '/items/gluttonous_pouch': {
    name: '贪食之袋',
  },
  '/items/gobo_boomstick': {
    name: '哥布林火棍',
  },
  '/items/gobo_boots': {
    name: '哥布林靴',
  },
  '/items/gobo_bracers': {
    name: '哥布林护腕',
  },
  '/items/gobo_chaps': {
    name: '哥布林皮裤',
  },
  '/items/gobo_defender': {
    name: '哥布林防御者',
  },
  '/items/gobo_essence': {
    name: '哥布林精华',
    description: '用于强化哥布林星球特殊装备的材料',
  },
  '/items/gobo_hide': {
    name: '哥布林皮',
  },
  '/items/gobo_hood': {
    name: '哥布林兜帽',
  },
  '/items/gobo_leather': {
    name: '哥布林皮革',
  },
  '/items/gobo_rag': {
    name: '哥布林抹布',
    description: '用于缝纫收藏家靴的材料',
  },
  '/items/gobo_shooter': {
    name: '哥布林弹弓',
  },
  '/items/gobo_slasher': {
    name: '哥布林关刀',
  },
  '/items/gobo_smasher': {
    name: '哥布林狼牙棒',
  },
  '/items/gobo_stabber': {
    name: '哥布林长剑',
  },
  '/items/gobo_tunic': {
    name: '哥布林皮衣',
  },
  '/items/goggles': {
    name: '护目镜',
    description: '用于锻造视觉头盔的材料',
  },
  '/items/golem_essence': {
    name: '魔像精华',
    description: '用于强化魔像洞穴特殊装备的材料',
  },
  '/items/gourmet_tea': {
    name: '美食茶',
  },
  '/items/granite_bludgeon': {
    name: '花岗岩大棒',
  },
  '/items/green_key_fragment': {
    name: '绿色钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/green_tea_leaf': {
    name: '绿茶叶',
  },
  '/items/griffin_bulwark': {
    name: '狮鹫重盾',
  },
  '/items/griffin_chaps': {
    name: '狮鹫皮裤',
  },
  '/items/griffin_leather': {
    name: '狮鹫之皮',
    description: '用于锻造狮鹫重盾跟缝纫狮鹫皮衣或皮裤的材料',
  },
  '/items/griffin_talon': {
    name: '狮鹫之爪',
    description: '用于锻造狮鹫重盾的材料',
  },
  '/items/griffin_tunic': {
    name: '狮鹫皮衣',
  },
  '/items/grizzly_bear_fluff': {
    name: '棕熊绒',
    description: '用于锻造棕熊鞋的材料',
  },
  '/items/grizzly_bear_shoes': {
    name: '棕熊鞋',
  },
  '/items/gummy': {
    name: '软糖',
  },
  '/items/guzzling_energy': {
    name: '暴饮能量',
    description: '用于缝纫暴饮之囊',
  },
  '/items/guzzling_pouch': {
    name: '暴饮之囊',
  },
  '/items/heal': {
    name: '自愈术',
  },
  '/items/holy_alembic': {
    name: '神圣蒸馏器',
  },
  '/items/holy_boots': {
    name: '神圣靴',
  },
  '/items/holy_brush': {
    name: '神圣刷子',
  },
  '/items/holy_buckler': {
    name: '神圣圆盾',
  },
  '/items/holy_bulwark': {
    name: '神圣重盾',
  },
  '/items/holy_cheese': {
    name: '神圣奶酪',
  },
  '/items/holy_chisel': {
    name: '神圣凿子',
  },
  '/items/holy_enhancer': {
    name: '神圣强化器',
  },
  '/items/holy_gauntlets': {
    name: '神圣护手',
  },
  '/items/holy_hammer': {
    name: '神圣锤子',
  },
  '/items/holy_hatchet': {
    name: '神圣斧头',
  },
  '/items/holy_helmet': {
    name: '神圣头盔',
  },
  '/items/holy_mace': {
    name: '神圣钉头锤',
  },
  '/items/holy_milk': {
    name: '神圣牛奶',
    description: '哞哞哞哞哞哞哞',
  },
  '/items/holy_needle': {
    name: '神圣针',
  },
  '/items/holy_plate_body': {
    name: '神圣胸甲',
  },
  '/items/holy_plate_legs': {
    name: '神圣腿甲',
  },
  '/items/holy_pot': {
    name: '神圣壶',
  },
  '/items/holy_shears': {
    name: '神圣剪刀',
  },
  '/items/holy_spatula': {
    name: '神圣锅铲',
  },
  '/items/holy_spear': {
    name: '神圣长枪',
  },
  '/items/holy_sword': {
    name: '神圣剑',
  },
  '/items/ice_spear': {
    name: '冰枪术',
  },
  '/items/icy_cloth': {
    name: '冰霜织物',
    description: '用于缝纫冰霜袍服或袍裙的材料',
  },
  '/items/icy_robe_bottoms': {
    name: '冰霜袍裙',
  },
  '/items/icy_robe_top': {
    name: '冰霜袍服',
  },
  '/items/impale': {
    name: '透骨之刺',
  },
  '/items/infernal_battlestaff': {
    name: '炼狱法杖',
  },
  '/items/infernal_ember': {
    name: '地狱余烬',
    description: '用于制作炼狱法杖的材料',
  },
  '/items/insanity': {
    name: '疯狂',
  },
  '/items/intelligence_coffee': {
    name: '智力咖啡',
  },
  '/items/invincible': {
    name: '无敌',
  },
  '/items/jackalope_antler': {
    name: '鹿角兔之角',
    description: '用于制作鹿角兔之杖的材料',
  },
  '/items/jackalope_staff': {
    name: '鹿角兔之杖',
  },
  '/items/jade': {
    name: '翡翠',
    description: '一种闪亮的宝石，通常在宝箱中找到',
  },
  '/items/jungle_essence': {
    name: '丛林精华',
    description: '用于强化丛林星球特殊装备的材料',
  },
  '/items/knights_aegis': {
    name: '骑士盾',
  },
  '/items/knights_ingot': {
    name: '骑士之锭',
    description: '用于锻造骑士之盾的材料',
  },
  '/items/kraken_chaps': {
    name: '克拉肯皮裤',
  },
  '/items/kraken_fang': {
    name: '克拉肯之牙',
    description: '用于制作涟漪、绽放或炽焰三叉戟的材料',
  },
  '/items/kraken_leather': {
    name: '克拉肯皮革',
    description: '用于缝纫克拉肯皮衣或皮裤的材料',
  },
  '/items/kraken_tunic': {
    name: '克拉肯皮衣',
  },
  '/items/large_artisans_crate': {
    name: '大工匠匣',
    description: '在生产时可以找到，看起来里面装着物品！',
  },
  '/items/large_meteorite_cache': {
    name: '大陨石舱',
    description: '在采集时可以找到，看起来里面装着物品！',
  },
  '/items/large_pouch': {
    name: '大袋子',
  },
  '/items/large_treasure_chest': {
    name: '大宝箱',
    description: '可以从怪物身上找到，看起来里面装着物品！',
  },
  '/items/liberica_coffee_bean': {
    name: '高级咖啡豆',
  },
  '/items/life_drain': {
    name: '生命吸取',
  },
  '/items/linen_boots': {
    name: '亚麻靴',
  },
  '/items/linen_fabric': {
    name: '亚麻布料',
  },
  '/items/linen_gloves': {
    name: '亚麻手套',
  },
  '/items/linen_hat': {
    name: '亚麻帽',
  },
  '/items/linen_robe_bottoms': {
    name: '亚麻袍裙',
  },
  '/items/linen_robe_top': {
    name: '亚麻袍服',
  },
  '/items/living_granite': {
    name: '花岗岩',
    description: '用于锻造花岗岩大棒或尖刺重盾的材料',
  },
  '/items/log': {
    name: '原木',
  },
  '/items/lucky_coffee': {
    name: '幸运咖啡',
  },
  '/items/lumber': {
    name: '木板',
  },
  '/items/lumberjacks_bottoms': {
    name: '伐木工下装',
  },
  '/items/lumberjacks_top': {
    name: '伐木工上衣',
  },
  '/items/luna_robe_bottoms': {
    name: '月神袍裙',
  },
  '/items/luna_robe_top': {
    name: '月神袍服',
  },
  '/items/luna_wing': {
    name: '月神翼',
    description: '用于缝纫月神袍服或袍裙的材料',
  },
  '/items/maelstrom_plate_body': {
    name: '怒涛胸甲',
  },
  '/items/maelstrom_plate_legs': {
    name: '怒涛腿甲',
  },
  '/items/maelstrom_plating': {
    name: '怒涛甲片',
    description: '用于锻造怒涛胸甲或腿甲的材料',
  },
  '/items/magic_coffee': {
    name: '魔法咖啡',
  },
  '/items/magicians_cloth': {
    name: '魔术师织物',
    description: '用于缝纫魔术师之帽的材料',
  },
  '/items/magicians_hat': {
    name: '魔术师帽',
  },
  '/items/magnet': {
    name: '磁铁',
    description: '用于锻造磁力手套的材料',
  },
  '/items/magnetic_gloves': {
    name: '磁力手套',
  },
  '/items/magnifying_glass': {
    name: '放大镜',
    description: '用于锻造视觉盾或缝纫瞄准护腕的材料',
  },
  '/items/maim': {
    name: '血刃斩',
  },
  '/items/mana_spring': {
    name: '法力喷泉',
  },
  '/items/manticore_shield': {
    name: '蝎狮盾',
  },
  '/items/manticore_sting': {
    name: '蝎狮之刺',
    description: '用于制作蝎狮盾的材料',
  },
  '/items/marine_chaps': {
    name: '航海皮裤',
  },
  '/items/marine_scale': {
    name: '海洋鳞片',
    description: '用于缝纫海洋皮衣或皮裤的材料',
  },
  '/items/marine_tunic': {
    name: '海洋皮衣',
  },
  '/items/marksman_bracers': {
    name: '神射护腕',
  },
  '/items/marksman_brooch': {
    name: '神射胸针',
    description: '用于缝纫神射护腕的材料',
  },
  '/items/marsberry': {
    name: '火星莓',
  },
  '/items/marsberry_cake': {
    name: '火星莓蛋糕',
  },
  '/items/marsberry_donut': {
    name: '火星莓甜甜圈',
  },
  '/items/medium_artisans_crate': {
    name: '中工匠匣',
    description: '在生产时可以找到，看起来里面装着物品！',
  },
  '/items/medium_meteorite_cache': {
    name: '中陨石舱',
    description: '在采集时可以找到，看起来里面装着物品！',
  },
  '/items/medium_pouch': {
    name: '中袋子',
  },
  '/items/medium_treasure_chest': {
    name: '中宝箱',
    description: '可以从怪物身上找到，看起来里面装着物品！',
  },
  '/items/milk': {
    name: '牛奶',
    description: '哞',
  },
  '/items/milking_essence': {
    name: '挤奶精华',
    description: '用于冲泡奶茶和制作炼金催化剂',
  },
  '/items/milking_tea': {
    name: '挤奶茶',
  },
  '/items/minor_heal': {
    name: '初级自愈术',
  },
  '/items/mirror_of_protection': {
    name: '保护之镜',
    description: '一种稀有神器，可在强化时作为任何保护装备的副本',
  },
  '/items/mooberry': {
    name: '哞莓',
  },
  '/items/mooberry_cake': {
    name: '哞莓蛋糕',
  },
  '/items/mooberry_donut': {
    name: '哞莓甜甜圈',
  },
  '/items/moolong_tea_leaf': {
    name: '哞龙茶叶',
  },
  '/items/moonstone': {
    name: '月亮石',
    description: '一种闪亮的宝石，通常在宝箱中找到',
  },
  '/items/natures_veil': {
    name: '自然菌幕',
  },
  '/items/necklace_of_efficiency': {
    name: '效率项链',
  },
  '/items/necklace_of_speed': {
    name: '速度项链',
  },
  '/items/necklace_of_wisdom': {
    name: '经验项链',
  },
  '/items/orange': {
    name: '橙子',
  },
  '/items/orange_gummy': {
    name: '橙子软糖',
  },
  '/items/orange_key_fragment': {
    name: '橙色钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/orange_yogurt': {
    name: '橙子酸奶',
  },
  '/items/panda_fluff': {
    name: '熊猫绒',
    description: '用于锻造熊猫手套的材料',
  },
  '/items/panda_gloves': {
    name: '熊猫手套',
  },
  '/items/peach': {
    name: '桃子',
  },
  '/items/peach_gummy': {
    name: '桃子软糖',
  },
  '/items/peach_yogurt': {
    name: '桃子酸奶',
  },
  '/items/pearl': {
    name: '珍珠',
    description: '一种闪亮的宝石，通常在宝箱中找到',
  },
  '/items/penetrating_shot': {
    name: '贯穿射击',
  },
  '/items/penetrating_strike': {
    name: '贯心之刺',
  },
  '/items/pestilent_shot': {
    name: '疫病射击',
  },
  '/items/philosophers_earrings': {
    name: '贤者耳环',
  },
  '/items/philosophers_necklace': {
    name: '贤者项链',
  },
  '/items/philosophers_ring': {
    name: '贤者戒指',
  },
  '/items/philosophers_stone': {
    name: '贤者之石',
    description: '传说中拥有无穷力量的石头',
  },
  '/items/pincer_gloves': {
    name: '蟹钳手套',
  },
  '/items/pirate_chest': {
    name: '海盗宝箱',
    description: '攻克【海盗基地】后的奖励，可以用【海盗宝箱钥匙】打开',
  },
  '/items/pirate_chest_key': {
    name: '海盗宝箱钥匙',
    description: '开启一个海盗宝箱',
  },
  '/items/pirate_entry_key': {
    name: '海盗钥匙',
    description: '允许进入地下城【海盗基地】1次',
  },
  '/items/pirate_essence': {
    name: '海盗精华',
    description: '用于强化海盗基地特殊装备的材料',
  },
  '/items/pirate_token': {
    name: '海盗代币',
    description: '来自【海盗基地】的地下城代币。可以在地下城商店里消费',
  },
  '/items/plum': {
    name: '李子',
  },
  '/items/plum_gummy': {
    name: '李子软糖',
  },
  '/items/plum_yogurt': {
    name: '李子酸奶',
  },
  '/items/poke': {
    name: '破胆之刺',
  },
  '/items/polar_bear_fluff': {
    name: '北极熊绒',
    description: '用于锻造北极熊鞋的材料',
  },
  '/items/polar_bear_shoes': {
    name: '北极熊鞋',
  },
  '/items/power_coffee': {
    name: '力量咖啡',
  },
  '/items/precision': {
    name: '精确',
  },
  '/items/prime_catalyst': {
    name: '至高催化剂',
    description: '在炼金时使用，可将任何行动的成功率提高 25% (乘法)。成功时消耗一个催化剂。',
  },
  '/items/processing_tea': {
    name: '加工茶',
  },
  '/items/provoke': {
    name: '挑衅',
  },
  '/items/puncture': {
    name: '破甲之刺',
  },
  '/items/purple_key_fragment': {
    name: '紫色钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/purpleheart_bow': {
    name: '紫心弓',
  },
  '/items/purpleheart_crossbow': {
    name: '紫心弩',
  },
  '/items/purpleheart_fire_staff': {
    name: '紫心火法杖',
  },
  '/items/purpleheart_log': {
    name: '紫心原木',
  },
  '/items/purpleheart_lumber': {
    name: '紫心木板',
  },
  '/items/purpleheart_nature_staff': {
    name: '紫心自然法杖',
  },
  '/items/purpleheart_shield': {
    name: '紫心盾',
  },
  '/items/purpleheart_water_staff': {
    name: '紫心水法杖',
  },
  '/items/purples_gift': {
    name: '小紫牛的礼物',
    description: '获得任务积分后小紫牛赠送的礼物，看起来里面装着物品！',
  },
  '/items/quick_aid': {
    name: '快速治疗术',
  },
  '/items/quick_shot': {
    name: '快速射击',
  },
  '/items/radiant_boots': {
    name: '光辉靴',
  },
  '/items/radiant_fabric': {
    name: '光辉布料',
  },
  '/items/radiant_fiber': {
    name: '光辉纤维',
  },
  '/items/radiant_gloves': {
    name: '光辉手套',
  },
  '/items/radiant_hat': {
    name: '光辉帽',
  },
  '/items/radiant_robe_bottoms': {
    name: '光辉袍裙',
  },
  '/items/radiant_robe_top': {
    name: '光辉袍服',
  },
  '/items/rain_of_arrows': {
    name: '箭雨',
  },
  '/items/rainbow_alembic': {
    name: '彩虹蒸馏器',
  },
  '/items/rainbow_boots': {
    name: '彩虹靴',
  },
  '/items/rainbow_brush': {
    name: '彩虹刷子',
  },
  '/items/rainbow_buckler': {
    name: '彩虹圆盾',
  },
  '/items/rainbow_bulwark': {
    name: '彩虹重盾',
  },
  '/items/rainbow_cheese': {
    name: '彩虹奶酪',
  },
  '/items/rainbow_chisel': {
    name: '彩虹凿子',
  },
  '/items/rainbow_enhancer': {
    name: '彩虹强化器',
  },
  '/items/rainbow_gauntlets': {
    name: '彩虹护手',
  },
  '/items/rainbow_hammer': {
    name: '彩虹锤子',
  },
  '/items/rainbow_hatchet': {
    name: '彩虹斧头',
  },
  '/items/rainbow_helmet': {
    name: '彩虹头盔',
  },
  '/items/rainbow_mace': {
    name: '彩虹钉头锤',
  },
  '/items/rainbow_milk': {
    name: '彩虹牛奶',
    description: '哞哞哞哞哞哞',
  },
  '/items/rainbow_needle': {
    name: '彩虹针',
  },
  '/items/rainbow_plate_body': {
    name: '彩虹胸甲',
  },
  '/items/rainbow_plate_legs': {
    name: '彩虹腿甲',
  },
  '/items/rainbow_pot': {
    name: '彩虹壶',
  },
  '/items/rainbow_shears': {
    name: '彩虹剪刀',
  },
  '/items/rainbow_spatula': {
    name: '彩虹锅铲',
  },
  '/items/rainbow_spear': {
    name: '彩虹长枪',
  },
  '/items/rainbow_sword': {
    name: '彩虹剑',
  },
  '/items/ranged_coffee': {
    name: '远程咖啡',
  },
  '/items/ranger_necklace': {
    name: '射手项链',
  },
  '/items/red_culinary_hat': {
    name: '红色厨师帽',
  },
  '/items/red_panda_fluff': {
    name: '小熊猫绒',
    description: '用于缝纫红色厨师帽或蓬松红帽的材料',
  },
  '/items/red_tea_leaf': {
    name: '红茶叶',
  },
  '/items/redwood_bow': {
    name: '红杉弓',
  },
  '/items/redwood_crossbow': {
    name: '红杉弩',
  },
  '/items/redwood_fire_staff': {
    name: '红杉火法杖',
  },
  '/items/redwood_log': {
    name: '红杉原木',
  },
  '/items/redwood_lumber': {
    name: '红杉木板',
  },
  '/items/redwood_nature_staff': {
    name: '红杉自然法杖',
  },
  '/items/redwood_shield': {
    name: '红杉盾',
  },
  '/items/redwood_water_staff': {
    name: '红杉水法杖',
  },
  '/items/regal_jewel': {
    name: '君王宝石',
    description: '用于锻造君王之剑和狂怒长枪的材料',
  },
  '/items/regal_sword': {
    name: '君王之剑',
  },
  '/items/rejuvenate': {
    name: '群体治疗术',
  },
  '/items/reptile_boots': {
    name: '爬行动物靴',
  },
  '/items/reptile_bracers': {
    name: '爬行动物护腕',
  },
  '/items/reptile_chaps': {
    name: '爬行动物皮裤',
  },
  '/items/reptile_hide': {
    name: '爬行动物皮',
  },
  '/items/reptile_hood': {
    name: '爬行动物兜帽',
  },
  '/items/reptile_leather': {
    name: '爬行动物皮革',
  },
  '/items/reptile_tunic': {
    name: '爬行动物皮衣',
  },
  '/items/revenant_anima': {
    name: '亡者之魂',
    description: '用于缝纫亡灵皮衣或皮裤的材料',
  },
  '/items/revenant_chaps': {
    name: '亡灵皮裤',
  },
  '/items/revenant_tunic': {
    name: '亡灵皮衣',
  },
  '/items/revive': {
    name: '复活',
  },
  '/items/ring_of_armor': {
    name: '护甲戒指',
  },
  '/items/ring_of_critical_strike': {
    name: '暴击戒指',
  },
  '/items/ring_of_essence_find': {
    name: '精华发现戒指',
  },
  '/items/ring_of_gathering': {
    name: '采集戒指',
  },
  '/items/ring_of_rare_find': {
    name: '稀有发现戒指',
  },
  '/items/ring_of_regeneration': {
    name: '恢复戒指',
  },
  '/items/ring_of_resistance': {
    name: '抗性戒指',
  },
  '/items/rippling_trident': {
    name: '涟漪三叉戟',
  },
  '/items/robusta_coffee_bean': {
    name: '中级咖啡豆',
  },
  '/items/rough_boots': {
    name: '粗糙靴',
  },
  '/items/rough_bracers': {
    name: '粗糙护腕',
  },
  '/items/rough_chaps': {
    name: '粗糙皮裤',
  },
  '/items/rough_hide': {
    name: '粗糙兽皮',
  },
  '/items/rough_hood': {
    name: '粗糙兜帽',
  },
  '/items/rough_leather': {
    name: '粗糙皮革',
  },
  '/items/rough_tunic': {
    name: '粗糙皮衣',
  },
  '/items/royal_cloth': {
    name: '皇家织物',
    description: '用于缝纫皇家袍服和皇家袍裙的材料',
  },
  '/items/royal_fire_robe_bottoms': {
    name: '皇家火系袍裙',
  },
  '/items/royal_fire_robe_top': {
    name: '皇家火系袍服',
  },
  '/items/royal_nature_robe_bottoms': {
    name: '皇家自然系袍裙',
  },
  '/items/royal_nature_robe_top': {
    name: '皇家自然系袍服',
  },
  '/items/royal_water_robe_bottoms': {
    name: '皇家水系袍裙',
  },
  '/items/royal_water_robe_top': {
    name: '皇家水系袍服',
  },
  '/items/scratch': {
    name: '爪影斩',
  },
  '/items/shard_of_protection': {
    name: '保护碎片',
    description: '从工匠匣中获得，用于合成保护之镜',
  },
  '/items/shield_bash': {
    name: '盾击',
  },
  '/items/shoebill_feather': {
    name: '鲸头鹳羽毛',
    description: '用于缝纫鲸头鹳鞋的材料',
  },
  '/items/shoebill_shoes': {
    name: '鲸头鹳鞋',
  },
  '/items/sighted_bracers': {
    name: '瞄准护腕',
  },
  '/items/silencing_shot': {
    name: '沉默之箭',
  },
  '/items/silk_boots': {
    name: '丝靴',
  },
  '/items/silk_fabric': {
    name: '丝绸',
  },
  '/items/silk_gloves': {
    name: '丝手套',
  },
  '/items/silk_hat': {
    name: '丝帽',
  },
  '/items/silk_robe_bottoms': {
    name: '丝绸袍裙',
  },
  '/items/silk_robe_top': {
    name: '丝绸袍服',
  },
  '/items/sinister_cape': {
    name: '阴森斗篷',
  },
  '/items/sinister_chest': {
    name: '阴森宝箱',
    description: '攻克【阴森马戏团】后的奖励，可以用【阴森宝箱钥匙】打开',
  },
  '/items/sinister_chest_key': {
    name: '阴森宝箱钥匙',
    description: '开启一个阴森宝箱',
  },
  '/items/sinister_entry_key': {
    name: '阴森钥匙',
    description: '允许进入地下城【阴森马戏团】1次',
  },
  '/items/sinister_essence': {
    name: '阴森精华',
    description: '用于强化阴森马戏团特殊装备的材料',
  },
  '/items/sinister_token': {
    name: '阴森代币',
    description: '来自【阴森马戏团】的地下城代币。可以在地下城商店里消费',
  },
  '/items/smack': {
    name: '重碾',
  },
  '/items/small_artisans_crate': {
    name: '小工匠匣',
    description: '在生产时可以找到，看起来里面装着物品！',
  },
  '/items/small_meteorite_cache': {
    name: '小陨石舱',
    description: '在采集时可以找到，看起来里面装着物品！',
  },
  '/items/small_pouch': {
    name: '小袋子',
  },
  '/items/small_treasure_chest': {
    name: '小宝箱',
    description: '可以从怪物身上找到，看起来里面装着物品！',
  },
  '/items/smoke_burst': {
    name: '烟爆灭影',
  },
  '/items/snail_shell': {
    name: '蜗牛壳',
    description: '用于锻造蜗牛壳头盔的材料',
  },
  '/items/snail_shell_helmet': {
    name: '蜗牛壳头盔',
  },
  '/items/snake_fang': {
    name: '蛇牙',
    description: '用于锻造蛇牙短剑的材料',
  },
  '/items/snake_fang_dirk': {
    name: '蛇牙短剑',
  },
  '/items/sorcerer_boots': {
    name: '巫师靴',
  },
  '/items/sorcerer_essence': {
    name: '法师精华',
    description: '用于强化巫师之塔特殊装备的材料',
  },
  '/items/sorcerers_sole': {
    name: '魔法师鞋底',
    description: '用于缝纫魔法师靴的材料',
  },
  '/items/soul_fragment': {
    name: '灵魂碎片',
    description: '用于制作灵魂猎手弩的材料',
  },
  '/items/soul_hunter_crossbow': {
    name: '灵魂猎手弩',
  },
  '/items/spaceberry': {
    name: '太空莓',
  },
  '/items/spaceberry_cake': {
    name: '太空莓蛋糕',
  },
  '/items/spaceberry_donut': {
    name: '太空莓甜甜圈',
  },
  '/items/spacia_coffee_bean': {
    name: '太空咖啡豆',
  },
  '/items/speed_aura': {
    name: '速度光环',
  },
  '/items/spike_shell': {
    name: '尖刺防护',
  },
  '/items/spiked_bulwark': {
    name: '尖刺重盾',
  },
  '/items/stalactite_shard': {
    name: '钟乳石碎片',
    description: '用于锻造石钟长枪或尖刺重盾的材料',
  },
  '/items/stalactite_spear': {
    name: '石钟长枪',
  },
  '/items/stamina_coffee': {
    name: '耐力咖啡',
  },
  '/items/star_fragment': {
    name: '星光碎片',
    description: '在陨石舱里发现的碎片，可以用来制作珠宝。',
  },
  '/items/star_fruit': {
    name: '杨桃',
  },
  '/items/star_fruit_gummy': {
    name: '杨桃软糖',
  },
  '/items/star_fruit_yogurt': {
    name: '杨桃酸奶',
  },
  '/items/steady_shot': {
    name: '稳定射击',
  },
  '/items/stone_key_fragment': {
    name: '石头钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/strawberry': {
    name: '草莓',
  },
  '/items/strawberry_cake': {
    name: '草莓蛋糕',
  },
  '/items/strawberry_donut': {
    name: '草莓甜甜圈',
  },
  '/items/stunning_blow': {
    name: '重锤',
  },
  '/items/sugar': {
    name: '糖',
  },
  '/items/sundering_crossbow': {
    name: '裂空之弩',
  },
  '/items/sundering_jewel': {
    name: '裂空宝石',
    description: '用于制作裂空之弩和锻造狂怒长枪的材料',
  },
  '/items/sunstone': {
    name: '太阳石',
    description: '一种闪亮的太阳形状的宝石',
  },
  '/items/super_alchemy_tea': {
    name: '超级炼金茶',
  },
  '/items/super_attack_coffee': {
    name: '超级攻击咖啡',
  },
  '/items/super_brewing_tea': {
    name: '超级冲泡茶',
  },
  '/items/super_cheesesmithing_tea': {
    name: '超级奶酪锻造茶',
  },
  '/items/super_cooking_tea': {
    name: '超级烹饪茶',
  },
  '/items/super_crafting_tea': {
    name: '超级制作茶',
  },
  '/items/super_defense_coffee': {
    name: '超级防御咖啡',
  },
  '/items/super_enhancing_tea': {
    name: '超级强化茶',
  },
  '/items/super_foraging_tea': {
    name: '超级采摘茶',
  },
  '/items/super_intelligence_coffee': {
    name: '超级智力咖啡',
  },
  '/items/super_magic_coffee': {
    name: '超级魔法咖啡',
  },
  '/items/super_milking_tea': {
    name: '超级挤奶茶',
  },
  '/items/super_power_coffee': {
    name: '超级力量咖啡',
  },
  '/items/super_ranged_coffee': {
    name: '超级远程咖啡',
  },
  '/items/super_stamina_coffee': {
    name: '超级耐力咖啡',
  },
  '/items/super_tailoring_tea': {
    name: '超级缝纫茶',
  },
  '/items/super_woodcutting_tea': {
    name: '超级伐木茶',
  },
  '/items/swamp_essence': {
    name: '沼泽精华',
    description: '用于强化沼泽星球特殊装备的材料',
  },
  '/items/sweep': {
    name: '重扫',
  },
  '/items/swiftness_coffee': {
    name: '迅捷咖啡',
  },
  '/items/sylvan_aura': {
    name: '自然光环',
  },
  '/items/tailoring_essence': {
    name: '缝纫精华',
    description: '用于冲泡缝纫茶和制作炼金催化剂',
  },
  '/items/tailoring_tea': {
    name: '缝纫茶',
  },
  '/items/tailors_bottoms': {
    name: '裁缝下装',
  },
  '/items/tailors_top': {
    name: '裁缝上衣',
  },
  '/items/task_crystal': {
    name: '任务水晶',
    description: '小紫牛给的水晶，可以用来制作特殊的饰品。',
  },
  '/items/task_token': {
    name: '任务代币',
    description: '任务代币。可在任务商店中使用这些代币',
  },
  '/items/taunt': {
    name: '嘲讽',
  },
  '/items/thread_of_expertise': {
    name: '专精之线',
    description: '用于制作特殊专业服装的材料',
  },
  '/items/tome_of_healing': {
    name: '治疗之书',
  },
  '/items/tome_of_the_elements': {
    name: '元素之书',
  },
  '/items/toughness': {
    name: '坚韧',
  },
  '/items/toxic_pollen': {
    name: '剧毒粉尘',
  },
  '/items/treant_bark': {
    name: '树皮',
    description: '用于制作树人盾的材料',
  },
  '/items/treant_shield': {
    name: '树人盾',
  },
  '/items/turtle_shell': {
    name: '乌龟壳',
    description: '用于锻造龟壳胸甲或腿甲的材料',
  },
  '/items/turtle_shell_body': {
    name: '龟壳胸甲',
  },
  '/items/turtle_shell_legs': {
    name: '龟壳腿甲',
  },
  '/items/twilight_essence': {
    name: '暮光精华',
    description: '用于强化暮光之城特殊装备的材料',
  },
  '/items/ultra_alchemy_tea': {
    name: '究极炼金茶',
  },
  '/items/ultra_attack_coffee': {
    name: '究极攻击咖啡',
  },
  '/items/ultra_brewing_tea': {
    name: '究极冲泡茶',
  },
  '/items/ultra_cheesesmithing_tea': {
    name: '究极奶酪锻造茶',
  },
  '/items/ultra_cooking_tea': {
    name: '究极烹饪茶',
  },
  '/items/ultra_crafting_tea': {
    name: '究极制作茶',
  },
  '/items/ultra_defense_coffee': {
    name: '究极防御咖啡',
  },
  '/items/ultra_enhancing_tea': {
    name: '究极强化茶',
  },
  '/items/ultra_foraging_tea': {
    name: '究极采摘茶',
  },
  '/items/ultra_intelligence_coffee': {
    name: '究极智力咖啡',
  },
  '/items/ultra_magic_coffee': {
    name: '究极魔法咖啡',
  },
  '/items/ultra_milking_tea': {
    name: '究极挤奶茶',
  },
  '/items/ultra_power_coffee': {
    name: '究极力量咖啡',
  },
  '/items/ultra_ranged_coffee': {
    name: '究极远程咖啡',
  },
  '/items/ultra_stamina_coffee': {
    name: '究极耐力咖啡',
  },
  '/items/ultra_tailoring_tea': {
    name: '究极缝纫茶',
  },
  '/items/ultra_woodcutting_tea': {
    name: '究极伐木茶',
  },
  '/items/umbral_boots': {
    name: '暗影靴',
  },
  '/items/umbral_bracers': {
    name: '暗影护腕',
  },
  '/items/umbral_chaps': {
    name: '暗影皮裤',
  },
  '/items/umbral_hide': {
    name: '暗影皮',
  },
  '/items/umbral_hood': {
    name: '暗影兜帽',
  },
  '/items/umbral_leather': {
    name: '暗影皮革',
  },
  '/items/umbral_tunic': {
    name: '暗影皮衣',
  },
  '/items/vampire_fang': {
    name: '吸血鬼之牙',
    description: '用于锻造吸血鬼短剑或制作吸血弓的材料',
  },
  '/items/vampire_fang_dirk': {
    name: '吸血鬼短剑',
  },
  '/items/vampiric_bow': {
    name: '吸血弓',
  },
  '/items/vampirism': {
    name: '吸血',
  },
  '/items/verdant_alembic': {
    name: '翠绿蒸馏器',
  },
  '/items/verdant_boots': {
    name: '翠绿靴',
  },
  '/items/verdant_brush': {
    name: '翠绿刷子',
  },
  '/items/verdant_buckler': {
    name: '翠绿圆盾',
  },
  '/items/verdant_bulwark': {
    name: '翠绿重盾',
  },
  '/items/verdant_cheese': {
    name: '翠绿奶酪',
  },
  '/items/verdant_chisel': {
    name: '翠绿凿子',
  },
  '/items/verdant_enhancer': {
    name: '翠绿强化器',
  },
  '/items/verdant_gauntlets': {
    name: '翠绿护手',
  },
  '/items/verdant_hammer': {
    name: '翠绿锤子',
  },
  '/items/verdant_hatchet': {
    name: '翠绿斧头',
  },
  '/items/verdant_helmet': {
    name: '翠绿头盔',
  },
  '/items/verdant_mace': {
    name: '翠绿钉头锤',
  },
  '/items/verdant_milk': {
    name: '翠绿牛奶',
    description: '哞哞',
  },
  '/items/verdant_needle': {
    name: '翠绿针',
  },
  '/items/verdant_plate_body': {
    name: '翠绿胸甲',
  },
  '/items/verdant_plate_legs': {
    name: '翠绿腿甲',
  },
  '/items/verdant_pot': {
    name: '翠绿壶',
  },
  '/items/verdant_shears': {
    name: '翠绿剪刀',
  },
  '/items/verdant_spatula': {
    name: '翠绿锅铲',
  },
  '/items/verdant_spear': {
    name: '翠绿长枪',
  },
  '/items/verdant_sword': {
    name: '翠绿剑',
  },
  '/items/vision_helmet': {
    name: '视觉头盔',
  },
  '/items/vision_shield': {
    name: '视觉盾',
  },
  '/items/watchful_relic': {
    name: '警戒遗物',
  },
  '/items/water_strike': {
    name: '流水冲击',
  },
  '/items/werewolf_claw': {
    name: '狼人之爪',
    description: '用于锻造狼人关刀或制作吸血弓的材料',
  },
  '/items/werewolf_slasher': {
    name: '狼人关刀',
  },
  '/items/wheat': {
    name: '小麦',
  },
  '/items/white_key_fragment': {
    name: '白色钥匙碎片',
    description: '看起来是某种钥匙的碎片，用来制作地下城钥匙',
  },
  '/items/wisdom_coffee': {
    name: '经验咖啡',
  },
  '/items/wisdom_tea': {
    name: '经验茶',
  },
  '/items/wizard_necklace': {
    name: '巫师项链',
  },
  '/items/woodcutting_essence': {
    name: '伐木精华',
    description: '用于冲泡伐木茶和制作炼金催化剂',
  },
  '/items/woodcutting_tea': {
    name: '伐木茶',
  },
  '/items/wooden_bow': {
    name: '木弓',
  },
  '/items/wooden_crossbow': {
    name: '木弩',
  },
  '/items/wooden_fire_staff': {
    name: '木制火法杖',
  },
  '/items/wooden_nature_staff': {
    name: '木制自然法杖',
  },
  '/items/wooden_shield': {
    name: '木盾',
  },
  '/items/wooden_water_staff': {
    name: '木制水法杖',
  },
  '/items/yogurt': {
    name: '酸奶',
  },
} as const
